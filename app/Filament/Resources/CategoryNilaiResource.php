...
class CategoryNilaiResource extends Resource
{
    ...
    protected static ?string $navigationTitle = 'Category Nilai';

    protected static ?string $slug = 'category-slug';
                ...
                Card::make()
                    ->schema([
                        TextInput::make('name')
                            ...
                            ->reactive()
                            ->required()
                            ->afterStateUpdated(fn(Set $set, ?string $state) => $set('slug', \Str::slug($state))), // Str dibiarkan karena tidak ada penggunaan
                        TextInput::make('slug')
                            ->required(),
                    ])
            ]);
